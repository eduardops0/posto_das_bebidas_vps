...
import {
  BarChart3, QrCode, FileText, Settings, Users, DollarSign, Shield, Package,
  ToggleLeft, ToggleRight, Moon, Sun, LogOut, User, ChevronUp, ChevronDown, UserCheck, Target, CheckSquare,ChartNoAxesCombined,Smartphone,Sheet,
  Tag, Store
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ url }: { url: string }) => {
...
const handleLogout = () => {
  ...
  React.useEffect(() => {
   ...
   const adminMenuItemsMenuItems = [
    { path: '/admin/dashboard', icon: BarChart3, label: 'Dashboard', gradient: 'from-primary to-secondary' },
    { path: '/admin/gerenciar-consultores', icon: Users, label: 'Gestão de Consultores', gradient: 'from-primary to-secondary' },
    { path: '/admin/gerenciador-de-planilhas', icon: Sheet, label: 'Gerenciar Planilhas', gradient: 'from-secondary to-primary'},
    { path: '/crm', icon: Package, label: 'CRM', gradient: 'from-secondary to-primary'},
    { path: '/registros', icon: ChartNoAxesCombined, label: 'Registros', gradient: 'from-secondary to-primary'},
    { path: '/admin/gerenciar-metas', icon: Target, label: 'Gerenciador de Metas', gradient: 'from-primary to-secondary' },
    { path: '/admin/fornecedores', icon: Store, label: 'Fornecedores', gradient: 'from-secondary to-primary' },
    { path: '/gerenciar-tags', icon: Tag, label: 'Gestão de Tags', gradient: 'from-secondary to-primary'},
    // { path: '/admin-settings', icon: Settings, label: 'Configurações', gradient: 'from-primary to-secondary' },
  ];
  const menuItems = adminMenuItemsMenuItems
;
  ...
  <div className="flex items-center space-x-2">
    <div className="w-8 h-8 rounded-xl overflow-hidden shadow-lg bg-gradient-primary flex items-center justify-center">
      {user.equipe_interna_detalhes?.profile_image ? (
        <img
          src={user.equipe_interna_detalhes.profile_image.startsWith('http')
            ? user.equipe_interna_detalhes.profile_image
            : `/${user.equipe_interna_detalhes.profile_image}`}
          alt={user.name}
